@extends('admin.components.base', ['titulo' => 'Editar Lista'])

@section('content')
    <div class="p-6 bg-white rounded-md shadow-md">
        <form id="form-edit" method="POST" action="{{ route('admin.lista.update', ['id' => $lista->id]) }}">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                <div>
                    ...
                    <input class="form-input w-full mt-2 rounded-md focus:border-indigo-600 border-2 py-2 px-2" type="text"
                        name="descripcion" value="{{ $lista->descripcion }}">
                </div>
        ...
        <div class="flex justify-between mt-4">
            <form id="delete-form" method="POST" action="{{ route('admin.lista.destroy', ['id' => $lista->id]) }}">
                @csrf
                @method('DELETE')
                <button
